...
export const TrashModal: React.FC<TrashModalProps> = ({
  ...
  const [filterType, setFilterType] = useState<'all' | 'khata' | 'rojmel' | 'product' | 'order' | 'expense'>('all');
  ...
  const filtered = trashList.filter(item => {
    ...
    else if (filterType === 'khata') matchType = item.type === 'khata_account' || item.type === 'khata_transaction';
    else if (filterType === 'rojmel') matchType = item.type === 'rojmel';
    else matchType = item.type === filterType;
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-3 animate-fade-in no-print">
      <div className="bg-white rounded-2xl max-w-3xl w-full max-h-[90vh] flex flex-col border-2 border-neutral-800 shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-blue-200 font-bold">
                ડિલીટ કરેલ ખાતા, વ્યવહારો, રોજમેળ, પ્રોડક્ટ અને બિલ અહીં સુરક્ષિત છે. તમે ગમે ત્યારે પાછા લાવી શકો છો!
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-3 bg-neutral-50 border-b border-neutral-200 flex flex-col sm:flex-row items-center justify-between gap-2.5 text-xs">
          {/* Filter Chips */}
          <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
            {[
              { id: 'all', label: 'બધા (All)', count: trashList.length },
              { id: 'khata', label: 'ઉધારી / ખાતાવહી', count: trashList.filter(t => t.type === 'khata_account' || t.type === 'khata_transaction').length },
              { id: 'rojmel', label: 'રોજમેળ', count: trashList.filter(t => t.type === 'rojmel').length },
              { id: 'product', label: 'પ્રોડક્ટ્સ', count: trashList.filter(t => t.type === 'product').length },
              { id: 'order', label: 'બિલ / ઓર્ડર્સ', count: trashList.filter(t => t.type === 'order').length },
              { id: 'expense', label: 'ખર્ચ', count: trashList.filter(t => t.type === 'expense').length }
            ].map(tab => (
              <button
                key={tab.id}
                type="button"
                onClick={() => setFilterType(tab.id as any)}
                className={`px-2.5 py-1.5 rounded-xl font-black whitespace-nowrap transition-all cursor-pointer border text-xs ${
                  filterType === tab.id
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <input
              ...
              className="px-3 py-1.5 bg-white border border-neutral-300 rounded-xl text-xs font-bold outline-none focus:border-blue-700 w-full sm:w-40"
            />
            {trashList.length > 0 && (
              <button
                type="button"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 max-h-[58vh]">
          {filtered.length > 0 ? (
            filtered.map(item => (
              <div
                key={item.id}
                className="bg-white p-3 rounded-xl border border-neutral-300 hover:border-neutral-400 shadow-2xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 transition-all"
              >
                <div className="flex items-start gap-3">
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border ${
                    item.type === 'khata_account' || item.type === 'khata_transaction'
                    ...
                    {(item.type === 'khata_account' || item.type === 'khata_transaction') && <Users className="w-5 h-5" />}
                    {item.type === 'rojmel' && <BookOpen className="w-5 h-5" />}
                  ...
                  <div>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-neutral-100 text-neutral-600 border border-neutral-200">
                        {item.type === 'khata_account'
                          ? 'ખાતું'
                          : item.type === 'khata_transaction'
                          ? 'ખાતા વ્યવહાર'
                          : item.type === 'rojmel'
                          ? 'રોજમેળ'
                          : item.type === 'product'
            ...
            <div className="text-center py-12 text-neutral-400 space-y-2">
              ...
              <p className="text-xs font-black text-neutral-600">ટ્રેશ બિન ખાલી છે!</p>
        ...
        <div className="p-3 bg-neutral-100 border-t border-neutral-200 flex items-center justify-between text-xs font-bold text-neutral-600">
          <span>💡 રીસ્ટોર કરવાથી ખાતું, વ્યવહાર, રોજમેળ કે પ્રોડક્ટ ફરીથી પોતાની જગ્યાએ પાછા ઉમેરાઈ જશે.</span>
          <button
            type="button"
            onClick={onClose}
